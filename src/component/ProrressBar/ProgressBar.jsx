import './ProgressBar.css'

export const ProgressBar = ({number, collected}) => {
    const collectedNum = number - collected;
    const remains = number - collectedNum;
    /*
     * const h это разница между суммой сбора и суммой собранных средвств
     */
    const h = Boolean(collected < number)

    function f(n) {
        return new Intl.NumberFormat("ru").format(n);
    }

    function percent() {
        return Math.round((remains / number) * 100);
    }

    const bgc = {background: 'linear-gradient(90deg, rgb(243, 165, 60), rgb(104, 92, 163))', width: `${percent()}%`}
    return (
        <div className="wrap" id="progress">
            <div className="line-header">
                <div className="left">
                    <div className="label">Собрано</div>
                    <div className="value"><span>{f(remains)}</span>
                        <svg width="13px" height="10.9px" viewBox="-1 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path fill="currentColor"
                                  d="m17.778 7.449c.002-.043.003-.093.003-.143 0-1.028-.425-1.956-1.11-2.618l-.001-.001c-.717-.658-1.678-1.061-2.732-1.061-.064 0-.128.002-.191.004h.009-5.454v7.636h5.454c.054.003.118.004.182.004 1.054 0 2.015-.403 2.735-1.064l-.003.003c.685-.663 1.111-1.591 1.111-2.619 0-.05-.001-.1-.003-.149v.007zm4.04 0c.003.076.005.164.005.253 0 2.006-.829 3.819-2.163 5.115l-.002.002c-1.371 1.292-3.224 2.086-5.262 2.086-.107 0-.213-.002-.319-.006h.015-5.795v2.011h8.608.021c.29 0 .525.235.525.525v.022-.001 2.182.021c0 .29-.235.525-.525.525-.007 0-.015 0-.022 0h.001-8.608v3.273.018c0 .147-.061.28-.16.374-.097.095-.23.154-.377.154-.002 0-.004 0-.007 0h-2.846c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-3.273h-3.818c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-2.182c0-.006 0-.014 0-.021 0-.29.235-.525.525-.525h.022-.001 3.818v-2.011h-3.818c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-2.54c0-.002 0-.004 0-.006 0-.147.059-.28.154-.377.094-.099.227-.16.374-.16h.019-.001 3.818v-10.72c0-.006 0-.014 0-.021 0-.29.235-.525.525-.525h.022-.001 9.187c.091-.004.197-.006.304-.006 2.038 0 3.891.794 5.266 2.09l-.004-.004c1.334 1.297 2.162 3.109 2.162 5.114 0 .088-.002.176-.005.264v-.013z"></path>
                        </svg>
                    </div>
                </div>
                <div className="right">
                    <div className="label">{h ? 'Осталось' : 'Сбор окончен'}</div>
                    <div className="value"><span>{h ? f(collectedNum) : ''}</span>
                        {h ?
                            <svg width="13px" height="10.9px" viewBox="-1 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path fill="currentColor"
                                      d="m17.778 7.449c.002-.043.003-.093.003-.143 0-1.028-.425-1.956-1.11-2.618l-.001-.001c-.717-.658-1.678-1.061-2.732-1.061-.064 0-.128.002-.191.004h.009-5.454v7.636h5.454c.054.003.118.004.182.004 1.054 0 2.015-.403 2.735-1.064l-.003.003c.685-.663 1.111-1.591 1.111-2.619 0-.05-.001-.1-.003-.149v.007zm4.04 0c.003.076.005.164.005.253 0 2.006-.829 3.819-2.163 5.115l-.002.002c-1.371 1.292-3.224 2.086-5.262 2.086-.107 0-.213-.002-.319-.006h.015-5.795v2.011h8.608.021c.29 0 .525.235.525.525v.022-.001 2.182.021c0 .29-.235.525-.525.525-.007 0-.015 0-.022 0h.001-8.608v3.273.018c0 .147-.061.28-.16.374-.097.095-.23.154-.377.154-.002 0-.004 0-.007 0h-2.846c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-3.273h-3.818c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-2.182c0-.006 0-.014 0-.021 0-.29.235-.525.525-.525h.022-.001 3.818v-2.011h-3.818c-.006 0-.014 0-.021 0-.29 0-.525-.235-.525-.525 0-.007 0-.015 0-.022v.001-2.54c0-.002 0-.004 0-.006 0-.147.059-.28.154-.377.094-.099.227-.16.374-.16h.019-.001 3.818v-10.72c0-.006 0-.014 0-.021 0-.29.235-.525.525-.525h.022-.001 9.187c.091-.004.197-.006.304-.006 2.038 0 3.891.794 5.266 2.09l-.004-.004c1.334 1.297 2.162 3.109 2.162 5.114 0 .088-.002.176-.005.264v-.013z"></path>
                            </svg> : ''}
                    </div>
                </div>
            </div>
            <div className="progress progress-bar-bordered">
                <div className="progress-bar" style={bgc}>
                    <span className="progress-value">{percent()} %</span>
                    <div className="strip"></div>
                </div>
            </div>
        </div>
    )
}